...
pub fn get_isle_compilations(
    ...
    let src_isa_ppc64 = codegen_crate_dir.join("src").join("isa").join("ppc64");
    ...
    IsleCompilations {
        items: vec![
            ...
            // The ppc64 instruction selector.
            IsleCompilation {
                name: "ppc64".to_string(),
                output: gen_dir.join("isle_ppc64.rs"),
                tracked_inputs: [
                    vec![prelude_isle.clone(), prelude_lower_isle.clone()],
                    lower_spec_inputs(&[]),
                    vec![
                        src_isa_ppc64.join("inst.isle"),
                        src_isa_ppc64.join("lower.isle"),
                    ],
                ]
                .concat(),
                untracked_inputs: vec![numerics_isle.clone(), clif_lower_isle.clone()],
